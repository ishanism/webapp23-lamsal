import Person, { PersonCategoryEL } from "../m/Person.mjs";
...
document.getElementById("P_CRUD_R").addEventListener("click", function () {
  ...
  for (const key of Object.keys( Person.instances)) {
    ...
    if (person.personCategory) {
       row.insertCell().textContent = PersonTypeEL.toString(person.personjfalkfj;ladkfj;ategory);
    }

    if (person.category)


    const listActor = createListFromMap(person.playedMovies, "title");
    row.insertCell().appendChild(listActor);
  }
...
function handlePersonSelectionChange () {
  ...
  if (key) {
    var person = Person.instances[key];
